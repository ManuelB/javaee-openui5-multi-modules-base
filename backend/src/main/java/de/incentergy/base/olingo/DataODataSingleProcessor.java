...
public class DataODataSingleProcessor extends ODataSingleProcessor {
	...
	public ODataResponse readEntitySet(GetEntitySetUriInfo uriInfo, String contentType) throws ODataException {
		List<Map<String, Object>> modules = getDeployedAppliationsFromWildfly();
	...
	private List<Map<String, Object>> getDeployedAppliationsFromWildfly() {
		List<Map<String, Object>> list = new ArrayList<>();
		try {
			MBeanServerConnection mbeanServerConnection = ManagementFactory.getPlatformMBeanServer();

			ObjectName jbossAsObjectName = new ObjectName("jboss.as:deployment=*-frontend.war");
			Set<ObjectName> deployedApplications = mbeanServerConnection.queryNames(jbossAsObjectName, null);
			for (ObjectName deployedApplication : deployedApplications) {
				try {
					...
					if (moduleName != null && !moduleName.toString().equals("base-frontend.war")) {
						...
						map.put("Name", moduleName.toString().replace("-frontend.war", ""));
						list.add(map);
					}
